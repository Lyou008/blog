...
if not secret_key:
    if app.debug or os.environ.get('FLASK_ENV') == 'development':
        ...
        raise ValueError(
            "❌ 生产环境必须设置 SECRET_KEY 环境变量！\n"
            "请运行：export SECRET_KEY='your-secret-key-here'"
        )
...
def get_db():
    """获取数据库连接（带重试）。支持 PostgreSQL 和 SQLite。"""
    db_url = os.environ.get('DATABASE_URL')
    
    # 如果没有 DATABASE_URL，使用本地 SQLite（开发环境）
    if not db_url:
    ...
    if 'render.com' not in db_url and 'postgres.render.com' not in db_url:
        print(f"警告：DATABASE_URL 可能不完整：{db_url[:60]}...")
        print("正确格式示例：postgresql://user:pass@dpg-xxx.oregon-postgres.render.com:5432/dbname")
        ...
        try:
            ...
            if i == 9:
                raise psycopg2.OperationalError(
                    f"无法连接数据库（已重试 10 次）：{e}\n"
                    f"当前 DATABASE_URL: {db_url[:100]}\n"
                    "请检查 Render Dashboard 中 Web Service 的 Environment 变量 DATABASE_URL 是否完整！"
                )
            time.sleep(2)
